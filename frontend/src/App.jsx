...
function AppShell({ session, isAuthenticated, onLogout, themeMode, onToggleTheme, children }) {
  ...
  const navItems = [
    { label: "Auctions", path: "/", icon: <GavelOutlinedIcon fontSize="small" /> },
  ...
  if (!isAuthenticated) {
    return (
      <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        <Container maxWidth="xl" sx={{ pt: 2.5, pb: 0 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            <Stack direction="row" spacing={1} alignItems="center">
              <AutoAwesomeOutlinedIcon color="primary" fontSize="small" />
              <Typography variant="subtitle2" color="text.secondary">Premium Auction Suite</Typography>
            </Stack>
    ...
    <BrowserRouter>
      ...
      >
          <Routes>
            ...
            />
            <Route path="/" element={isAuthenticated ? <RFQList role={session.role} /> : <Navigate to="/login" replace />} />
            <Route path="/profile" element={isAuthenticated ? <Profile /> : <Navigate to="/login" replace />} />
            <Route
              path="/create"
              element={isAuthenticated && session.role === 'buyer' ? <CreateRFQ /> : <Navigate to="/" replace />}
            />
            <Route
              path="/metrics"
              element={isAuthenticated && session.role === "buyer" ? <BuyerMetrics /> : <Navigate to="/" replace />}
            />
